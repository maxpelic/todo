...
class Entry{
    ...
    function getEntries($userId, $jobId){
        return $this->formatResult($this->sql->prepare('SELECT * FROM entries WHERE userId = ? AND jobId = ? ORDER BY clockedIn DESC')->bindString($userId)->bindString($jobId)->execute()->getResult());
    }
